...
export default function presetExtension(pi: ExtensionAPI) {
  ...
  async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
    ...
    if (preset.tools) {
      ...
      // ensure that there is a list of valid tools to pass or that the
      // `preset.tools` list has been manually overridden in the config to be
      // empty, turning off all tools
      if (validTools.length > 0 || preset.tools.length === 0) {
