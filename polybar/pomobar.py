...
class GlobalPomodoroCounter:
    ...
    def increment_global_pomodoro_counter():
        ...
        try:
            connection = sqlite3.connect(DATABASE_PATH)
            cursor = connection.cursor()
            cursor.execute("UPDATE POMODOROS SET COMPLETADOS = (" "" + pomodoros_actualizados + "" ") WHERE COMPLETADOS = '" + pomodoros_completados + "'")
            connection.commit()
            connection.close()
        except Exception as e:
            print(e)
...
def stop():
    ...
    writeOutput(ICON_TOTAL + GlobalPomodoroCounter.get_pomodoros_done())
    recharge_polybar()
    os.system("notify-send --urgency=low Session cancelled")
